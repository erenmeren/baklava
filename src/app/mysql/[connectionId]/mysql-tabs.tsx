...
type Tab =
  | { kind: "overview" }
  | { kind: "processlist" }
  | { kind: "table"; db: string; name: string }
...
function tabHref(connectionId: string, t: Tab): string {
  switch (t.kind) {
    ...
    case "processlist":
      return `/mysql/${connectionId}/processlist`;
    case "table":
      return `/mysql/${connectionId}/databases/${encodeURIComponent(t.db)}/tables/${encodeURIComponent(t.name)}`;
    case "query":
      return `/mysql/${connectionId}/databases/${encodeURIComponent(t.db)}/query/${t.queryId}`;
  }
}
...
function tabFromPath(pathname: string, connectionId: string): Tab | null {
  const prefix = `/mysql/${connectionId}`;
  if (!pathname.startsWith(prefix)) return null;
  const rest = pathname.slice(prefix.length);
  if (rest === "" || rest === "/") return { kind: "overview" };
  if (rest === "/processlist" || rest.startsWith("/processlist")) {
    return { kind: "processlist" };
  }
  // /databases/[db]/tables/[name]
  const tableMatch = rest.match(/^\/databases\/([^/]+)\/tables\/([^/]+)/);
  if (tableMatch) {
    return {
      kind: "table",
      db: decodeURIComponent(tableMatch[1]),
      name: decodeURIComponent(tableMatch[2]),
    };
  }
...
export function MysqlTabs({ connectionId, defaultDatabase }: Props) {
  ...
  const { tabs, setTabs, hydrated, activeKey, closeTab } = useTableTabs<Tab>({
    storageKey: `baklava:my-tabs:${connectionId}`,
    activeTab,
    key: tabKey,
    href: (t) => tabHref(connectionId, t),
    homeHref: `/mysql/${connectionId}`,
    onAdd: (tab, existing) =>
      tab.kind === "query"
  ...
  const queryDb = defaultDatabase || "_";
...
function Tab({
  ...
  return (
    <div
      className={cn(
        "group/mytab relative inline-flex items-stretch h-9 max-w-[260px] min-w-0",
        "transition-colors",
        ...
        const iconNode = (
          <span
            className={cn(
              ...
              active ? "opacity-100" : "opacity-70 group-hover/mytab:opacity-100",
            )}
          >
            {icon}
          </span>
      ...
      {showClose && onClose && !editing ? (
        <button
          ...
          className={cn(
            ...
            "text-muted-foreground/0 group-hover/mytab:text-muted-foreground/70 focus-visible:text-foreground",
