...
export default async function DaemonSetsPage({ params, searchParams }: Props) {
  ...
  return result.ok ? (
    <DaemonSetsView list={result.list} />
  ) : (
    <LoadError resource="daemon sets" error={result.error} />
  );
}
