...
**Rulings taken while implementing Tasks 10–11** — the interface above is the sketch; these are the shapes it actually needed, and why.

1. **The shell owns the Data tab, not just the schema tabs.** Left in each client's `render.data`, the Postgres and MySQL data tabs stay ~150-line near-duplicates (toolbar, grid, row actions, pagination, the two error branches), and no client reaches criterion 6's 400-line bar. So the descriptor grew a `data: DataTabSpec` — `fetch`, `columns`, `schemaTab`, plus `toolbar` / `sortable` / `insertClassName` / `gridClassName` flags for the three real divergences. Rows are normalized to `DataPage { fields, rows, total }`, which is what flattens MySQL's object rows.
2. **`capabilities` is three flags, not eight.** `insertRow` / `editRow` / `deleteRow` are what the shell renders and therefore what it can gate. `truncate` / `dropTable` / `createIndex` / `dropIndex` / `renameIndex` are per-tech header buttons and dialogs the shell never sees; a flag it never reads is dead configuration. Per-*table* gating (Postgres and MySQL need a primary key) is a separate `rowsMutable(all)` predicate, because the descriptor is built before the columns load.
3. **`render` receives one args object, and it carries `all`.** `(data, ctx)` is not enough: the Postgres Structure tab's FK chips come from a *second* tab's payload, and its Statistics panel counts columns and indexes. So `render[tab]({ ctx, data, all, ctl })`. Two more optional maps came with it — `toolbar[tab]` for the Indexes tab's "N indexes · New index" bar, which must stay clickable while the list is loading or has failed, and `skeleton[tab]` / `contentClassName[tab]` for the placeholder and layout differences.
4. **Children are a render prop, and `title` / `description` / `actions` accept one too.** The client's dialogs need the loaded schema (RowFormDialog's columns, ModifyTableDialog's, the delete PK) and a refresh handle — all now inside the shell. `children?(ctl)` and `HeaderSlot = ReactNode | ((ctl) => ReactNode)` hand it over, rather than a ref.
5. **`per-tab` needed `eager` and `prefetch`.** Postgres loads `structure` on mount whatever tab is open (the Data tab's header hints, the PK markers and both mutation buttons depend on it) and pulls `foreign_keys` when the Structure tab opens. Neither is expressible as pure per-tab laziness, and the lazy-DDL characterization test still passes because nothing else is eager.
6. **In-flight requests are tracked, and the containers are lazy `useState`, not `useRef`.** The cache-null + no-error guard stays satisfied while a request is in the air, so a re-render in that window would issue a duplicate — and three suites count requests exactly. `react-hooks/refs` (eslint) forbids handing a ref-reading callback to children during render, which `ctl.reloadData` is, so the `Map`/`Set` live in `useState(() => new …)`.
7. **`reloadData` takes an optional offset.** SQL Server's insert previously reset to page 1; preserving that needs a way to say "reload, but at offset 0", distinct from Retry's "reload where the user is".

**New co-located files beyond the plan's list**, all of them per-tech panels or dialogs moved out of a client so the client can be a descriptor (criterion 6's intended end state): `{postgres,mysql,sqlserver}/…/table-types.ts` (row shapes, formatters, SQL Server's `buildClientDdl`), `{postgres,mysql,sqlserver}/…/meta-columns.tsx` (the `MetaTable` panels), `postgres/…/index-dialogs.tsx` (index rename/drop), `mysql/…/table-actions.tsx` (truncate / drop-table / drop-index behind one dialog), and the shared `src/components/workspace/confirm-dialog.tsx`.
